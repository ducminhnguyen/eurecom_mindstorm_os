...
void runStraightLine(struct MotorInfo motorInfo, struct SensorInfo sensorInfo) {
    ...
    if (sensorInfo.diffGyro > 2 && left_motor_speed < right_motor_speed) { // left tilt
        ...
        set_tacho_speed_sp(motorInfo.rightMotor, motorInfo.speed - 20*sensorInfo.diffGyro);
        ...
        set_tacho_speed_sp(motorInfo.leftMotor, motorInfo.speed - 20*sensorInfo.diffGyro);
        printf("Right tilt: %f \n", sensorInfo.diffGyro);
    } else {
        set_tacho_speed_sp(motorInfo.leftMotor, motorInfo.speed);
        set_tacho_speed_sp(motorInfo.rightMotor, motorInfo.speed);
